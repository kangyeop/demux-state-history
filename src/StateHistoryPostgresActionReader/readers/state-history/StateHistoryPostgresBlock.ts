...
export class StateHistoryPostgresBlock implements Block {
  ...
  public async parseActions() {
    const eosApi = this.getApi(this.blockInfo.blockNumber, this.massiveInstance, this.dbSchema, this.log)
  ...
  public getApi = (blockNumber: number, massiveInstance: any, dbSchema: string, log: Logger) => {
    const instanceAbiProvider = this.api.abiProvider as StateHistoryPostgresAbiProvider
