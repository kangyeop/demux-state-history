...
export class StateHistoryPostgresBlock implements Block {
    ...
    constructor(
        ...
        const abiProvider = new StateHistoryPostgresAbiProvider(ledEndpoint);
    ...
    public async parseActions() {
        const eosApi = this.getApi(
            this.blockInfo.blockNumber,
            this.massiveInstance,
            this.dbSchema,
            this.log
        );
        const filteredActionTraces = this.actionTraceAuthorizations.filter(
            (actionTrace: any) => {
                if (
                    this.defaultIgnoreActions.includes(
                        `${actionTrace.act_account}::${actionTrace.act_name}`
                    )
                ) {
                    return false;
                }
        ...
        const indexedFirstActions = await this.getIndexedFirstActions(
            eosApi,
            filteredActionTraces
        );

        const actionPromises: Array<Promise<
            EosAction
        >> = filteredActionTraces.map(
    ...
    public getApi = (
        ...
        const instanceAbiProvider = this.api
            .abiProvider as StateHistoryPostgresAbiProvider;
        instanceAbiProvider.setState(
            blockNumber,
            massiveInstance,
            dbSchema,
            log
    ...
    private warnIfNotDistinct() {
        const globalSequences = [
            ...new Set(
                this.actionTraceAuthorizations.map(
                    (action: any) => action.receipt_global_sequence
                )
            )
        ];
        if (
            !(globalSequences.length === this.actionTraceAuthorizations.length)
        ) {
            console.warn(
                `Global action traces are not distinct (block number ${this.blockInfo.blockNumber})`
            );
        }
    }
    ...
    private async getIndexedFirstActions(eosApi: Api, actionTraces: any) {
        const toProcessFirst: { [key: string]: any } = actionTraces.reduce(
            (accumulator: any, actionTrace: any, index: number) => {
                if (
                    !Object.keys(accumulator).includes(actionTrace.act_account)
                ) {
        ...
        const firstActionsPromises = Object.values(toProcessFirst).map(
            async (indexedActionTrace: any) => {
                const action = await this.getEosAction(
                    eosApi,
                    indexedActionTrace.actionTrace
                );
                return {
                    action,
        ...
        return firstActions.reduce((accumulator: any, firstAction: any) => {
    ...
    private createSerializedAction(actionTrace: any) {
        return {
            ...
            authorization: actionTrace.authorizations.map(
    ...
    private async getEosAction(eosApi: Api, actionTrace: any): Promise<any> {
        ...
        try {
            [deserializedAction] = await eosApi.deserializeActions([
                serializedAction
            ]);
        } catch (err) {
            if (err.message.startsWith("Unknown action")) {
                ...
                try {
                    ...
                    if (err.message.startsWith("Unknown action")) {
                        console.warn(
                            `Action ${actionTrace.act_account}::${actionTrace.act_name} does not have an ABI; ` +
        ...
        const action: EosAction = {
            ...
            payload: {
                ...
                isNotification:
                    actionTrace.act_account !== actionTrace.receipt_receiver,
                contextFreeData:
                    this.contextFreeDataById[actionTrace.transaction_id] || []
            }
        };
    ...
    private attachTransactionActions(actionsByTxId: {
        [key: string]: EosAction[];
    }) {
        for (const transactionId of Object.keys(actionsByTxId)) {
            ...
            const contextFreeData =
                actionsByTxId[transactionId][0].payload.contextFreeData;
    ...
    private getContextFreeDataById() {
        this.contextFreeDataById = this.rawContextFreeData.reduce(
            (accumulator: any, cfd: any) => {
